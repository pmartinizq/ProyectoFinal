...
void functionHandler(void){
			...
			//tomo el parametro de getMessure
		  	...
		  	switch ( parameter ) {
				...
				case ULTRASONIC_LEFT:
					...
					//Llamo la funcion que toma la medicion
					//ultrasonicLeftMessure(ULTRASONIC_LEFT);
				  	break;
  ...
  f1.timerCount=44;
				  	setToExecutingVector(&f1);
				  	//setToExecutingVector(&functionStructInstance);
					

					//Llamo la funcion que toma la medicion
					//ultrasonicMessure(ULTRASONIC_RIGHT);
				  	break;
				...
				case ULTRASONIC_FRONT:
				  	...
				  	functionStructInstance.functionId = ULTRASONIC_FRONT;
				  	functionStructInstance.status = RUNNING;
				  	functionStructInstance.timerCount = ULTRASONIC_FRONT_TIMER;
  ...
  f1.root=UNIQUE_FUNCTION;
  f1.timerCount=50;
              newFunction=newFunctionInExecutingVector();
              newFunction->functionId=ULTRASONIC_FRONT;
              newFunction->status=RUNNING;
              newFunction->root=UNIQUE_FUNCTION;
              newFunction->timerCount=10;              
				  	//setToExecutingVector(&f1);
				  	//setToExecutingVector(&functionStructInstance);							
            //Sensor1(&functionStructInstance);
					//Llamo la funcion que toma la medicion
					//ultrasonicMessure(ULTRASONIC_FRONT);
				  	break;
